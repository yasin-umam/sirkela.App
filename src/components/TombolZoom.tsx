...
function TombolZoom({ zoom, zoomIn, zoomOut, reset, bisaZoomIn, bisaZoomOut, default: sudahDefault }: ReturnType<typeof useZoomKontrol>) {
  return (
    <div className="flex flex-col items-center gap-0.5 rounded-2xl bg-white/95 backdrop-blur shadow-lg border border-slate-200 p-1">
      <button
        type="button"
        onClick={zoomIn}
        disabled={!bisaZoomIn}
        title="Perbesar"
        className="w-8 h-8 rounded-xl flex items-center justify-center text-slate-600 font-bold text-base leading-none active:bg-slate-100 disabled:opacity-30 disabled:active:bg-transparent transition-colors"
      >
        +
      </button>
      <button
        ...
        className="w-8 h-8 flex items-center justify-center text-[10px] font-semibold text-slate-500 active:bg-slate-100 rounded-lg disabled:active:bg-transparent transition-colors"
      >
        {Math.round(zoom * 100)}%
      </button>
      <button
